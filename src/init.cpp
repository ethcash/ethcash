...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ethcash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ethcashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeethcashamount=<n> " + _("Keep N EthCash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. EthCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EthCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("EthCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "EthCash server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of EthCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart EthCash to complete") << "\n";
    ...
    nAnonymizeethcashAmount = GetArg("-anonymizeethcashamount", 0);
    if(nAnonymizeethcashAmount > 999999) nAnonymizeethcashAmount = 999999;
    if(nAnonymizeethcashAmount < 2) nAnonymizeethcashAmount = 2;
    ...
    LogPrintf("Anonymize EthCash Amount %d\n", nAnonymizeethcashAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ETHCA+1000 == (.1ETHCA+100)*10
       10ETHCA+10000 == (1ETHCA+1000)*10
    */

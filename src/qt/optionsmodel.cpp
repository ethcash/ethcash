...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeethcashAmount"))
        settings.setValue("nAnonymizeethcashAmount", 1000);
    nAnonymizeethcashAmount = settings.value("nAnonymizeethcashAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeethcashAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeethcashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeethcashAmount:
            return QVariant(nAnonymizeethcashAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeethcashAmount:
            nAnonymizeethcashAmount = value.toInt();
            settings.setValue("nAnonymizeethcashAmount", nAnonymizeethcashAmount);
            emit AnonymizeethcashAmountChanged(nAnonymizeethcashAmount);
            break;
        default:
            break;
        }

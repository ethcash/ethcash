...
{
    // Convert ethcash:// to ethcash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ethcash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ethcash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EthCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EthCash.lnk
...
{
    return GetAutostartDir() / "ethcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ethcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EthCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ethcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ethcash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("EthCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ethcash-qt [" + tr("command-line options") + "]                     " + "\n";

...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeethcashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeethcashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeethcashAmount = strAnonymizeethcashAmount.remove(strAnonymizeethcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeethcashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeethcashAmount*COIN) nMaxToAnonymize = nAnonymizeethcashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeethcashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeethcashAmount));
        strAnonymizeethcashAmount = strAnonymizeethcashAmount.remove(strAnonymizeethcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeethcashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
